const registerValidDriver = (
  ...
  const registerValidPoliceOfficer = (
    Email,
    Fname,
    Lname,
    Password,
  ...
  const fineValidator = (
    ...
    if (!fineID) return "No fine ID Generated!";
    if (!vehicleNumber) return "Please enter vehicle Number!";
    if (!place) return "Please enter Location";
    if (!fineDateTime) return "Please enter Date & Time!";
    if (!natureOfOffence) return "Please enter Nature of Offence!";
  };
